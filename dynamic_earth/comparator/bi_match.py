...
def extract_features(image: np.ndarray, model, processor, model_config: dict, device: str) -> torch.Tensor:
    ...
    if model_type == 'DINO':
        ...
        h, w = processed_image.shape[-2:] # 512,512
        features = model.get_intermediate_layers(processed_image)[0] # torch.Size([1, 1025, 768]) -> 1*1024*768
        features = features[:, 1:].permute(0, 2, 1).view(1, feature_dim, h // patch_size, w // patch_size)  # torch.Size([1, 768, 32, 32])
    elif model_type == 'DINOv2':
        
        processed_image = processor(images=image, return_tensors="pt").to(device)
        h, w = processed_image['pixel_values'].shape[-2:] # 512,512
        ...
        # torch.Size([1, 1296, 768]) -> torch.Size([1, 768, 36, 36])
...
def bitemporal_match(
    ...
    img1_embed = extract_features(img1, model, processor, model_config, device) # torch.Size([1, 768, 18, 18])
    ...
    H, W = img1.shape[:2] # 256*256
    img1_embed = F.interpolate(img1_embed, size=(H, W), mode='bilinear', align_corners=True).squeeze_(0) # torch.Size([768, 256, 256]), not normalized
    ...
    def _latent_match(mask_data: np.ndarray, img1_embed: torch.Tensor, img2_embed: torch.Tensor):
        ...
        change_confidence = torch.zeros(len(mask_data), dtype=torch.float32, device=device) # One score per mask

        for i, mask in enumerate(mask_data): # Pool masked features from both times and compare them
        ...
        keep_indices = change_confidence > angle_to_cosine(change_confidence_threshold) # Convert the angle threshold to cosine space
        keep_indices = keep_indices.cpu().numpy() # Boolean keep flags for all masks
        retained_mask_data = mask_data[keep_indices]
        retained_count_img1 = len(np.where(keep_indices[:img1_mask_num])[0]) # Number of retained masks from image 1

        return retained_mask_data, retained_count_img1

    # Perform latent matching and return results
    
    filtered_mask_data, filtered_img1_mask_num = _latent_match(mask_data, img1_embed, img2_embed) # Example shape: 174 x 256 x 256
