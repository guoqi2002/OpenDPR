...
def load_change_location_label(file_name, shape):
    """Load the change location label and convert it to a binary mask."""
    label_path = os.path.join(CHANGE_LABEL_DIR, file_name)
    label = imread(label_path)
    if label.ndim == 3:
        label = label[..., 0]

    binary_label = (label == 255).astype(np.uint8)
    if binary_label.shape != shape:
        raise ValueError(
            f"Change label shape {binary_label.shape} does not match image shape {shape}: {label_path}"
        )
    return binary_label


def filter_masks_by_change_label(change_masks, binary_label):
    """Keep masks that overlap enough with the change location label."""
    valid_masks = []
    for mask in change_masks:
        mask = mask.astype(bool)
        total_mask_pixels = np.sum(mask)
        overlap_pixels = np.sum(np.logical_and(mask, binary_label))

        if total_mask_pixels > 0 and overlap_pixels / total_mask_pixels > MIN_LABEL_OVERLAP_RATIO:
            valid_masks.append(mask)

    if valid_masks:
        return np.stack(valid_masks)
    return np.empty((0, *binary_label.shape), dtype=bool)


def apply_iou_filter(change_mask, binary_label):
    """Filter the change mask by IoU with the change location label."""
    intersection = np.sum(np.logical_and(binary_label, change_mask))
    union = np.sum(np.logical_or(binary_label, change_mask))
...
CHANGE_LABEL_DIR = "S2C_results/WHU"
NUM_CLASSES = 2
...
for file_name in tqdm(target_files, desc="Processing", unit="iteration"):
    ...
    binary_label = load_change_location_label(file_name, img1.shape[:2])
    cmasks = filter_masks_by_change_label(cmasks, binary_label)

    change_mask = merge_masks(cmasks, img1.shape[:2])
    change_mask = apply_iou_filter(change_mask, binary_label)
